...
* `regulations` への書き込み API が存在しない。読み出し(`GET /api/tournaments/:tournamentId/regulations`)だけが実装済みで、レギュレーション本体も優先エントリー期間も Supabase を直接操作しないと設定できない状態になっている(`docs/api-endpoints.md` 13章「未実装のエンドポイント」)。要件の中核である「複数条件のレギュレーション」「優先期間中は対象者のみエントリー可」が運用できないため、これを塞ぐ。
