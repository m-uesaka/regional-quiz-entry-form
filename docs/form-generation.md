...
- `checkbox`: `options` は任意。**あれば複数選択のチェックボックス群、なければ単独のブールチェックボックス**(「規約に同意します」など)になります。
...
- 単独ブールチェックボックス(`options` なし)は、「自分のキー1つだけを選択肢に持つ」ものとして照合されます。つまり `[field.key]` か `[]` 以外は弾かれます。
