...
**キューへの投入に失敗した場合は `500` を返しますが、その時点で投入済みの宛先には送信されます。** 投入は `sendBatch()` 何回かに分かれるため、途中で失敗しても前半は既にキューに乗っており、取り消す手段はありません。積めなかった宛先はジョブの `failed` に計上するので(そうしないと `sent + failed` が `total` に届かず、いつまでも送信中に見えます)、**`500` を受け取ったら再送する前に `GET .../mail/:jobId` で何人に届いたかを確認してください**。そのまま同じ内容を送り直すと、前半の宛先には 2 通届きます。
