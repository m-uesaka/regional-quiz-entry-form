...
この2つは構造的に重なっている(どちらも `error` フィールドを持つ)ため、フロントエンドで応答を判定するときは `res.status` ではなく**フィールドの有無と型**で絞り込んでください。
...
- 繰り上げ処理が失敗しても**リクエスト自体は成功扱い**にし、ログ出力にとどめます。キャンセル自体は既にコミット済みであり、ここで失敗を返すと「キャンセルできていない」と参加者に誤解させるためです。失われるのは繰り上げ(またはその通知メール)だけで、これはスタッフが後から再実行できます。
