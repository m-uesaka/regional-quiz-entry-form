...
- **参加者アカウント(participant)** は「メールアドレス + パスワード」で識別され、1つの地域にひもづく(地域をまたいだ参加ができないため)。同じ地域内で同じ participant が複数の大会(最強位・新人王)に entry を持てるかは地域ごとの設定(`regions.allows_dual_entry`、既定 false)で決まる(Task 10-1)。
...
graph TD
  ...
  P9 --> P10["Phase 10: 要件との差分の解消"]:::wip
  P8 --> P11["Phase 11: セキュリティ強化"]:::wip
  P9 --> P11
...
```mermaid
graph TD
  T111["11-1 レート制限/Turnstile"]:::done --> T112["11-2 メール列挙対策"]:::next
...
* [Task 11-1: レート制限と Turnstile ✅](tasks/task-11-1.md) — ログイン 2 本にレート制限、エントリー登録とパスワード再設定要求にレート制限と Turnstile を入れた
