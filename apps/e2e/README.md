...
### Turnstile はテスト用キーで通す

エントリーフォームとパスワード再設定要求フォームには Cloudflare Turnstile が入っています(#116)。テスト実行では Cloudflare が公開している**テスト用キーの組**を使います(`support/env.ts`)。サイトキー側は操作なしで必ず解けるウィジェットを描画し、シークレット側は siteverify がどんなトークンも受け入れます。参加者と同じようにフォームを埋めて送信するだけで通るので、スペック側に Turnstile 用の分岐はありません(`support/ui.ts` がトークンの到着だけ待ちます)。

そのため **テスト実行中は `challenges.cloudflare.com` に到達できる必要があります**。ブラウザがウィジェットのスクリプトを取りに行き、Worker が同じホストでトークンを検証するためです。ここが塞がれている環境では、エントリー系のスペックがトークンなしの送信として 400 で落ちます。
