...
// deliberate — there is no API for creating staff accounts (see the
// "未実装" section of `docs/api-endpoints.md`), which is exactly what a run
// needs to start from, and the rows that do have one are inserted the same
// way to keep the seed a single pass over Postgres.
